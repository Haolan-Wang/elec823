...
class BetterEar(nn.Module):
    ...
    def forward(self, x):
        x = self.relu(self.fc1(x))   # Apply ReLU activation to first layer output
        x = self.fc2(x)              # No activation function on the final output layer
        return x 



class HearingImpairment(nn.Module):
    """Input: (batch_size, 160, 601)
...
class HearingImpairmentFC(nn.Module):
    """Input: (batch_size, 160, 601)
        Output: (batch_size, 128, 151)
    """
    def __init__(self):
        super(HearingImpairmentFC, self).__init__()
        self.fc1 = nn.Linear(160 * 601, 512)
        self.ReLU = nn.ReLU()
...
class EncoderPredictorFCHI(nn.Module):
    def __init__(self):
        super(EncoderPredictorFCHI, self).__init__()
        pretrained_model = nemo_asr.models.EncDecCTCModel.from_pretrained(
            "nvidia/stt_en_conformer_ctc_large"
        )
        self.logmel = pretrained_model.preprocessor
        self.hearing_impairment = HearingImpairmentFC()
        self.conformer_encoder = pretrained_model.encoder
        self.predictor = nn.Sequential(
            nn.Linear(in_features=640 * 151, out_features=256),
            nn.ReLU(),
            nn.Linear(in_features=256, out_features=128),
            nn.ReLU(),
            nn.Linear(in_features=128, out_features=1)
    ...
    def forward(self, speech_l, speech_r, meta_data):
        ...
        # mel_out: [B, 80, 601]
        mel_feature_l, _ = self.logmel(
            input_signal=speech_l.to(device),
            length=torch.full((speech_l.shape[0],), speech_l.shape[1]).to(device),
        )
        mel_feature_r, _ = self.logmel(
            input_signal=speech_r.to(device),
            length=torch.full((speech_r.shape[0],), speech_r.shape[1]).to(device),
        )
        # encoder out : [B, 512, 151]
        encoded_l, _ = self.conformer_encoder(
            audio_signal=mel_feature_l.to(device),
            length=torch.full((mel_feature_l.shape[0],), mel_feature_l.shape[2]).to(device),
        )
        encoded_r, _ = self.conformer_encoder(
            audio_signal=mel_feature_r.to(device),
            length=torch.full((mel_feature_r.shape[0],), mel_feature_r.shape[2]).to(device),
        )
        
        concat_mel_feature_l = torch.cat((mel_feature_l, interpolated_audiogram_l), dim=1) # [B, 160, 601]
        concat_mel_feature_r = torch.cat((mel_feature_r, interpolated_audiogram_r), dim=1)
        # impaired_feature: [B, 128, 151]
        impaired_feature_l = self.hearing_impairment(concat_mel_feature_l)
        impaired_feature_r = self.hearing_impairment(concat_mel_feature_r)
        
        # [encoded_l, impaired_feature_l] -> [B, 640, 151]
        # [encoded_r, impaired_feature_r] -> [B, 640, 151]
        concat_feature_l = torch.cat((encoded_l, impaired_feature_l), dim=1) 
        concat_feature_r = torch.cat((encoded_r, impaired_feature_r), dim=1)
        pred_l = self.predictor(concat_feature_l.contiguous().view(-1, 640 * 151))
        pred_r = self.predictor(concat_feature_r.contiguous().view(-1, 640 * 151))
        
        stacked_pred = torch.stack((pred_l, pred_r), dim=1).squeeze(2)
        pred = self.mapping(self.better_ear(stacked_pred))

        return pred


class EncoderPredictorFCHINOBE(nn.Module):
    # No better ear: l,r concat -> predictor
    def __init__(self):
        super(EncoderPredictorFCHINOBE, self).__init__()
        pretrained_model = nemo_asr.models.EncDecCTCModel.from_pretrained(
            "nvidia/stt_en_conformer_ctc_large"
        )
        self.logmel = pretrained_model.preprocessor
        ...
        self.conformer_encoder = pretrained_model.encoder
        self.predictor = nn.Sequential(
            nn.Linear(in_features=1280 * 151, out_features=256),
            nn.ReLU(),
            nn.Linear(in_features=256, out_features=128),
            nn.ReLU(),
            nn.Linear(in_features=128, out_features=1)
    ...
    def forward(self, speech_l, speech_r, meta_data):
        ...
        concat_feature_l = torch.cat((encoded_l, impaired_feature_l), dim=1) 
        concat_feature_r = torch.cat((encoded_r, impaired_feature_r), dim=1)
        concat_feature = torch.cat((concat_feature_l, concat_feature_r), dim=1)
        pred = self.predictor(concat_feature.contiguous().view(-1, 1280 * 151))
...
class EncoderPredictorCNNHIBE(nn.Module):
    def __init__(self):
        super(EncoderPredictorCNNHIBE, self).__init__()
        pretrained_model = nemo_asr.models.EncDecCTCModel.from_pretrained(
            "nvidia/stt_en_conformer_ctc_large"
        )
        self.logmel = pretrained_model.preprocessor
    ...
    def forward(self, speech_l, speech_r, meta_data):
        ...
        # [encoded_l, impaired_feature_l] -> [B, 1024, 151]
        # [encoded_r, impaired_feature_r] -> [B, 1024, 151]
        concat_feature_l = torch.cat((encoded_l, impaired_feature_l), dim=1) 
        concat_feature_r = torch.cat((encoded_r, impaired_feature_r), dim=1)
        pred_l = self.predictor(concat_feature_l.contiguous().view(-1, 1024 * 151))
        pred_r = self.predictor(concat_feature_r.contiguous().view(-1, 1024 * 151))
        
        stacked_pred = torch.stack((pred_l, pred_r), dim=1).squeeze(2)
        # pred = self.mapping(self.better_ear(stacked_pred))
        pred, _ = torch.max(stacked_pred, dim=1)
        pred = self.mapping(pred)
